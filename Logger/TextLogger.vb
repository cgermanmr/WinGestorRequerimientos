...
Public Class TextLogger
    ...
    'RETURNS:       Nothing
    '*************************************************************
    Public Shared Sub WriteToTextLog(ByVal msg As String, _
                                     ByVal stkTrace As String, _
                                     ByVal title As String, _
                                     Optional ByVal idSuseso As Integer = 0)
        ...
        SyncLock objLock
            ...
            'check the file
            Dim fs As FileStream = New FileStream(Application.StartupPath & "\log\logs.txt", FileMode.OpenOrCreate, FileAccess.ReadWrite)
            Dim s As StreamWriter = New StreamWriter(fs)
            s.Close()
            fs.Close()

            'log it
            Dim fs1 As FileStream = New FileStream(Application.StartupPath & "\log\logs.txt", FileMode.Append, FileAccess.Write)
            Dim s1 As StreamWriter = New StreamWriter(fs1)
            s1.Write("Title: " & title & vbCrLf)
            s1.Write("Id: " & idSuseso & vbCrLf)
